...
credentials:
  - key: GEMINI_API_KEY
    ...
    description: "Get your key at ai.google.dev"
    required: true
---
...
**Before running the script**, read the API key from macOS Keychain:

```bash
security find-generic-password -s superbot2-plugin-credentials -a "nano-banana-pro/GEMINI_API_KEY" -w
```

Then pass the result via `--api-key`:

```bash
uv run ~/.claude/skills/nano-banana-pro/scripts/generate_image.py --api-key "$(security find-generic-password -s superbot2-plugin-credentials -a 'nano-banana-pro/GEMINI_API_KEY' -w)" --prompt "..." --filename "..."
```

If the Keychain lookup fails (exit code 44 = not found), fall back to the `GEMINI_API_KEY` environment variable. If neither is available, tell the user to configure the key in the superbot2 dashboard (Skills → nano-banana-pro → Credentials).
